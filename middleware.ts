...
export async function middleware(request: NextRequest) {
  ...
  const isConnectorCommand = request.nextUrl.pathname.startsWith("/api/attendance/commands");

  if (isApi && !user && !isWorkerSync && !isConnectorIngest && !isConnectorCommand) {
  ...
  if (user && (isDashboard || isApi)) {
    ...
    if (profile?.role === "employee") {
      const employeeDefaultRoute = request.nextUrl.pathname === "/dashboard"
      ...
      const permission = module && profile.employee_id
        ? await supabase.from("permissions").select("can_read, can_add, can_edit, can_delete").eq("employee_id", profile.employee_id).eq("module", module).maybeSingle()
        : { data: null };
      const requiredPermission = requiredAction(request);
      const allowed = employeeDefaultRoute
        || (permission.data?.[requiredPermission] === true);

      if (!allowed && isApi) return NextResponse.json({ error: `Permission required: ${module ?? "this module"}.` }, { status: 403 });
      if (!allowed && isDashboard) return NextResponse.redirect(new URL("/dashboard", request.url));
    }
  }
...
function requiredAction(request: NextRequest): "can_read" | "can_add" | "can_edit" | "can_delete" {
