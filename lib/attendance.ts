...
function officeDateTimeToUtc(date: string, minutes: number) {
  return new Date(new Date(`${date}T00:00:00Z`).getTime() - officeOffsetMinutes * 60000 + minutes * 60000).toISOString();
}
...
function overnightMidpoint(shift: Shift) {
  return Math.floor((timeToMinutes(shift.end_time) + timeToMinutes(shift.start_time)) / 2);
}
...
export function dateKey(value: string | Date) {
  const shifted = new Date(new Date(value).getTime() + officeOffsetMinutes * 60000);
...
export function sessionDateKey(value: string | Date, shift?: Shift) {
  ...
  const office = new Date(new Date(value).getTime() + officeOffsetMinutes * 60000);
...
export function sessionWindow(date: string, shift?: Shift) {
  if (isOvernightShift(shift) && shift) {
    ...
    return { start: officeDateTimeToUtc(date, midpoint), end: officeDateTimeToUtc(addDays(date, 1), midpoint) };
  }
...
export function buildAttendanceDays({ employees, shifts, punches, approvedLeaves, startDate, endDate }: { employees: Employee[]; shifts: Shift[]; punches: Punch[]; approvedLeaves: { employee_id: number; start_date: string; end_date: string }[]; startDate: string; endDate: string }) {
  ...
  punches.forEach((punch) => {
    const employee = (punch.employee_id ? employeeById.get(punch.employee_id) : undefined) ?? employeeByUid.get(Number(punch.zk_user_id));
    if (!employee) return;
  ...
  return dates.flatMap((date) => employees.map((employee) => {
    const employeePunches = sessionPunches(grouped.get(`${employee.id}:${date}`) ?? []);
