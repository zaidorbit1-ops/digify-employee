...
function isIncomingRecord(value: unknown): value is IncomingRecord {
  ...
  if (record.user_id === undefined || record.record_time === undefined) return false;
  const userId = Number(record.user_id);
  const timestamp = new Date(String(record.record_time));
  return Number.isInteger(userId) && userId > 0 && Number.isFinite(timestamp.getTime());
}

export async function POST(request: Request) {
  if (!hasValidToken(request)) {
    return NextResponse.json({ ok: false, error: "Invalid connector authentication." }, { status: 401 });
  }

  try {
    const body = await request.json();
    const deviceIp = typeof body?.device_ip === "string" ? body.device_ip.trim() : "";
    const port = Number(body?.port);
    const records = body?.records;

    if (!deviceIp || !Number.isInteger(port) || port <= 0 || !Array.isArray(records) || records.length > 10000) {
      return NextResponse.json({ ok: false, error: "A valid device_ip, port, and records array are required." }, { status: 400 });
    }
    if (!records.every(isIncomingRecord)) {
      return NextResponse.json({ ok: false, error: "Each attendance record must contain a valid user_id and record_time." }, { status: 400 });
    }

    const [devices, employees, shifts] = await Promise.all([getDevices(), getEmployees(), getShiftTimings()]);
    const device = devices.find((item) => item.device_ip === deviceIp && Number(item.port) === port);
    const rawRecords = records.map((record) => ({
      ...normalizeAttendanceRecord({ ...record, user_id: String(record.user_id) }),
    ...
    const synced = attendanceRecords.length ? await upsertAttendanceRecords(attendanceRecords) : [];
    ...
    return NextResponse.json({ ok: true, received: records.length, synced: synced.length });
  } catch (error) {
    console.error("[ATTENDANCE INGEST] failed", error instanceof Error ? error.message : "Unknown ingest error");
    return NextResponse.json({ ok: false, error: "Attendance ingest failed." }, { status: 500 });
  }
}

export async function GET() {
  return NextResponse.json({ ok: true, message: "Use POST with connector authentication to ingest attendance." });
}