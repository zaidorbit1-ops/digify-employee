...
export async function GET() {
  try {
    ...
    const { data, error } = await client.from("salaries").select("id, month, base_salary, late_days, absent_days, half_days, deduction_amount, net_pay, status, paid_at, adjustment_note").eq("employee_id", profile.employee_id).order("month", { ascending: false });
