...
export default function EmployeesPage() {
  ...
  async function handleEnroll(employee: Employee) {
    ...
    try {
      const response = await fetch("/api/employees/enroll", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ employee_id: employee.id, device_id: deviceId, zk_device_uid: employee.zk_device_uid ?? employee.id }) });
      const result = await response.json(); if (!response.ok) throw new Error(`${result.error ?? "Enrollment failed."} ${result.hint ?? ""}`.trim());
      setMessage({ text: result.message ?? `${employee.name} was enrolled successfully.`, tone: "success" }); await loadData();
