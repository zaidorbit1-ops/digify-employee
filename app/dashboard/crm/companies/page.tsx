...
export default function CrmCompaniesPage() {
  ...
  async function saveIntegration(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setIntegrationMessage({ text: "Integration created successfully. Save the secret below now.", tone: "success" });
  ...
  return <>
    ...
    <Modal open={Boolean(integrationWebsite)} onClose={() => { if (!integrationLoading) setIntegrationWebsite(null); }} title="Create integration identity" description={`Create a secure identity for ${integrationWebsite?.name ?? "this website"}.`}><form className="space-y-5" onSubmit={saveIntegration}>{integrationMessage ? <Alert tone={integrationMessage.tone}>{integrationMessage.text}</Alert> : null}<Field label="Integration name"><TextInput required disabled={integrationLoading || Boolean(integrationSecret)} value={integrationName} onChange={(event) => setIntegrationName(event.target.value)} placeholder="Website lead form" /></Field><p className="text-xs leading-5 text-muted">A secret will be generated after creation and shown once. Store it securely for the website integration.</p>{integrationSecret ? <div className="rounded-xl border border-amber-200 bg-amber-50 p-4"><p className="text-sm font-semibold text-amber-900">Save this secret now</p><p className="mt-1 text-xs text-amber-800">It will not be shown again.</p><p className="mt-3 break-all rounded-lg bg-white px-3 py-2 font-mono text-xs text-foreground">{integrationSecret}</p></div> : null}<div className="flex justify-end gap-3 border-t border-border pt-5"><Button type="button" variant="secondary" disabled={integrationLoading} onClick={() => setIntegrationWebsite(null)}>Close</Button>{!integrationSecret ? <Button type="submit" disabled={integrationLoading || !integrationName.trim()}>{integrationLoading ? "Creating identity..." : "Create identity"}</Button> : null}</div></form></Modal>
  </>;
}